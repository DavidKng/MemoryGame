...
export const Board = () => {
   ...
   const createBoard = () => {
    ...
    const cartas= newCards.map((card) => {
        return {
            ...card,
            flipped: false,
            matched: false,
   ...
   const handleCardClick= (id) => {
     ...
     if(!currentCard.flipped && !currentCard.matched){
        ...
        const newFlipeddCards= [...flippedCards, currentCard];
        setFlippedCards(newFlipeddCards);

        if(newFlipeddCards.length === 2 ){
            setIsDisabled(true);
            const[firstCard, secondCard] = newFlipeddCards;
